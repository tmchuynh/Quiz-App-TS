...
const quizDataMap: { [key: string]: Quiz; } = {
...
type QuizDataKey = typeof quizData[number]["id"];
...
function promptForDifficulty( quizId: string ) {
...
function setupQuizData( quizId: string, difficultyLevel: number ) {
	removeAllSections();
	const currentUserId = sessionStorage.getItem( "currentUserId" )!;
	...
	const quizType = quizOptions.find( option => option.id === quizId )?.label || "Quiz";
